...
def test_connection():
    ...
    # Use assert instead of return False
    assert access_key, "Missing KALSHI_ACCESS_KEY in .env file"
    assert private_key, "Missing KALSHI_PRIVATE_KEY in .env file"
    ...
    assert prices is not None, "Failed to retrieve prices"
    ...
    assert position is not None, "Failed to retrieve position"
